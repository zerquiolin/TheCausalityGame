...
\game is not a metaphor but a rigorously specified interactive system for causal discovery and reasoning grounded in structural causal models (SCMs) \citep{pearl2009causality}. 
...
This section elevates that intuition into a mathematical object, so that every experiment discussed later inherits unambiguous semantics. We refer to Appendix~\ref{appendix:math_foundations} for a complete derivation of the run semantics and Pareto-based evaluation.
...
The agent never observes $\scm$ or any $\latentx$; instead, it interacts solely through interventions on $\controllablex$ and receives measured outcomes from $\measuredx \sqcup \controllablex$. 
This information asymmetry mirrors scientific experimentation: one can manipulate controllable instruments and measure select sensors, yet the generative law remains latent.
...
Appendix~\ref{appendix:specs_registry} elaborates how these access modes are encoded declaratively through Specs.

\subsection{Environment and Experiment Execution}
The environment is a deterministic causal oracle induced by the extended SCM. It does not maintain any mutable internal ``state’’ beyond the initialization seed. Instead, it responds to each agent action by \emph{functionally generating new data} and returning it to the agent. All memory of past interactions is externalized into the \textbf{Transcript}, which is maintained by the \game orchestrator, not by the environment itself. This separation simplifies theoretical analysis: the environment can be viewed as a pure structural simulator, while the transcript captures the Markovian history required for evaluation.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item a \texttt{RoundInformation} object summarizing the current budget status, mission description, and other contractual metadata,
...
\end{enumerate}
The inform–act cycle is intentionally reminiscent of reinforcement-learning turn taking, yet it differs fundamentally: the environment is not a reward-emitting MDP but a causal oracle that only yields samples consistent with $\scm$ \citep{von2019optimal,Seitzer2021CausalIDA,Mehrjou2020CausalCRA}.
...
In practical deployments—clinical trials, field experiments, or robotics platforms—such budget adherence is often a legal or safety obligation \citep{Prosperi2020CausalIAA,chengcausaltime}. The game codifies that constraint without privileging any algorithm.
...
\quad \text{with Pareto-optimality well-defined.}
\]
However, no scalarization is imposed at the framework level — users may rank agents by convex combination, lexicographic ordering, or full Pareto frontier analysis. This preserves transparency about whether an agent succeeds through frugality, accuracy, or both.
