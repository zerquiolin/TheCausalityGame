...
The SCM is the \emph{only source of ground-truth causality} — every observation, every experiment outcome, and every dependency experienced by the agent originates here. Designing compelling benchmarks therefore requires curating SCMs whose provenance, expressiveness, and controllability reflect the challenges faced by practitioners \citep{pearl2009causality,reisach2021beware}.

While its formal definition was provided in Section~\ref{sec:formaldefinition}, we now examine the SCM from the viewpoint of the \textbf{runtime engine}—where it becomes an \emph{active, queryable data-generating process}. We highlight three complementary sources that populate \package’s library: factual datasets, curated Bayesian networks, and physics-inspired structural laws.
...
\]
The implementation guarantees that sampling remains thread-safe and seed-deterministic, so that concurrent benchmark runs never interfere (see Appendix~\ref{appendix:reproducibility}).
...
\package ships three datasets from which SCMs can be derived, summarized in Table~\ref{tab:scm_datasets}.
...
\paragraph{Synthesis.}
Collectively, these sources give rise to dozens of SCM families whose topologies, functional forms, and noise processes vary widely—from low-dimensional binary systems to continuous networks with nonlinear interactions. Because each SCM is accompanied by an explicit access partition $(\latentx,\measuredx,\controllablex)$ and randomized seeds, researchers can reproduce community baselines, design ablation studies, or contribute new structural scenarios with confidence that they integrate seamlessly into the benchmark. This diversity is essential for evaluating agents that claim to generalize beyond a single domain \citep{RealCause2021,BayesianCausalDiscovery2023}.
