"""Behavior metric that accounts for rounds, samples, and memory usage (stateless)."""
...
class InvalidResourceWeightsError(ValueError):
    """Raised when all resource weights are non-positive."""

    def __init__(self) -> None:
        super().__init__("At least one resource weight must be positive.")
...
class ResourceUsageBehaviorMetric(BehaviorMetric):
    ...
    def evaluate(self, transcript: Transcript) -> float:
        ...
        # Rounds
        score += self._rounds_weight * self._percentage_used(
            used=(
        ...
        # Samples
        score += self._samples_weight * self._percentage_used(
            used=(
        ...
        score += self._time_weight * self._percentage_used(
            used=time_used_s,
        ...
        score += self._memory_weight * self._percentage_used(
            used=memory_used_mb,
    ...
    def _percentage_used(self, used: int | float, total: int | float) -> float:
        return (used * 100.0) / total if total > 0.0 else 0.0
